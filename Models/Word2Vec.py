...
eval_words = ['brake', 'engine', 'battery', 'software', 'car', 'vehicle']
...
for file in onlyfiles:
    for line in pd.read_csv(file, delimiter=',', error_bad_lines=False)["processedReviews"]:
        text_words.append(str(line).lower().split())

# Build the dictionary and replace rare words with UNK token
count = [('UNK', -1)]
# Retrieve the most common words
count.extend(collections.Counter(list(chain(*text_words))).most_common(max_vocabulary_size-1))
...
print("Unique words:", len(set(chain(*text_words))))
